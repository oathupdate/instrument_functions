...
class CygProfile {
 public:
    NO_INSTRUMENT static CygProfile &instance();
    NO_INSTRUMENT CygProfile();
    NO_INSTRUMENT std::string &get(void *addr);
...
CygProfile &CygProfile::instance() {
    static CygProfile p = CygProfile();
    return p;
}

std::string &CygProfile::get(void *addr) {
...
static std::string &GetTraceFunctionName(void *callee, void *caller) {
